...
В Obsidian можно использовать различные типы заметок для выделения информации разными способами. Вот некоторые из них:

`[!note]` - для обычных заметок.
> [!note] Заметка

`>[!abstract]` или `[!summary]` - для резюме или краткого содержания.
>[!abstract] Содержание

 `>[!info]` - для информации.
>[!info] Для информации

 `>[!tip]` - для советов или подсказок.
>[!tip] Совет

 `>[!success]` - для успешных результатов или позитивных замечаний.
>[!success] Удачно
...
>[!warning] Внимание
 ...
 `>[!failure]` - для неудач или ошибок.
>[!failure] Неудачно

 `>[!danger]` - для обозначения опасности.
>[!danger] Опасно
