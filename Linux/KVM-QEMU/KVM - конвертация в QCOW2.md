...
Преобразование образа виртуальной машины VirtualBox VM DVI в формат образа KVM Qcow2 - еще один удобный и мощный метод переноса образа виртуальной машины VirtualBox в Linux KVM. Сначала нам нужно установить `virt machine manager` (диспетчер виртуальных машин) в наших дистрибутивах Linux, позволяющий нам выполнять задачи преобразования изображений.
...
> Команда конвертации образа VirtualBox VMDK в DVI .
```shell
vboxmanage clonehd box-disk1.vmdk ubuntu.vdi --format vdi
```

>Команда конвертации образа DVI в Qcow2 для использования в KVM Linux.
```shell
qemu-img convert -f vdi -O qcow2 ubuntu.vdi ubuntu.qcow2
```

После конвертации вы сможете использовать файл образа qcow2 на вашем KVM Linux.  Если при запуске что-то пойдет не так, перезагрузите компьютер и повторите попытку конвертации.
# Заключение
