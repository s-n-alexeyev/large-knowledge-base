...
`-logfile СТРОКА [путь до файла]` - Записывать журнал приложения в казанный файл
...
`-service СТРОКА` - Управление системны сервисом: ["start" "stop" "restart" "install" "uninstall"]
...
```
### Возможные проблемы
При обновлении пакета листнер сокета может поменяться на ListenStream=127.0.2.1:53 вместо ListenStream=127.0.0.1:53
...
>Запретить от прослушки 52 порта
```shell
